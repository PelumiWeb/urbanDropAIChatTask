...
import { Colors, Ease, Fonts, Gutter, Motion } from '@/constants/theme';
import {
  LATENCY_MS,
  dealResults,
  greeting,
...
export default function DropAIScreen() {
  ...
  const ask = useCallback((text: string, intent?: Intent) => {
    ...
    setMessages((current) => [
      ...current,
      { id: nextId.current++, kind: 'user', text: query },
      { id: nextId.current++, kind: 'thinking' },
    ]);
      ...
      <KeyboardAvoidingView
        ...
        {typeaheadOpen ? (
          <Animated.View
            ...
            entering={SlideInDown.duration(220).easing(Ease)}>
